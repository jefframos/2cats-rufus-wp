...
{
    class FXGreenSmoke : DefaultFX         
    {
        public FXGreenSmoke(float x, float y) :base(x,y)
        {
        }
        public override void init(ContentManager Content)
        {
            texture = Content.Load<Texture2D>(".\\Sprites\\effects\\getBolinha\\FXBolinha");
            spriteSheetPositions = Content.Load<Dictionary<string, Rectangle>>(".\\Sprites\\effects\\getBolinha\\FXBolinhaXML");
            spriteSheet = new SpriteSheet(texture, spriteSheetPositions);
            spriteSheet.addAnimation(new AnimationStruct("INIT", AnimationStruct.makeSequence(0, 9), 10, kill));
