...
{
    class FXGetMush : DefaultFX         
    {
        public FXGetMush(float x, float y)
            : base(x, y)
        {
        }
        public override void init(ContentManager Content)
        {
            texture = Content.Load<Texture2D>(".\\Sprites\\effects\\getItem1\\getItem");
            spriteSheetPositions = Content.Load<Dictionary<string, Rectangle>>(".\\Sprites\\effects\\getItem1\\getItemXML");
            spriteSheet = new SpriteSheet(texture, spriteSheetPositions);
            spriteSheet.addAnimation(new AnimationStruct("INIT", AnimationStruct.makeSequence(0, 4), 10, kill));
