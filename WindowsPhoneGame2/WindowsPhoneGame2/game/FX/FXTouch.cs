...
{
    class FXTouch : DefaultFX         
    {
        public FXTouch(float x, float y)
            : base(x, y)
        {
        }
        public override void init(ContentManager Content)
        {
            texture = Content.Load<Texture2D>(".\\Sprites\\effects\\touch\\FXTouch");
            spriteSheetPositions = Content.Load<Dictionary<string, Rectangle>>(".\\Sprites\\effects\\touch\\FXTouchXML");
            spriteSheet = new SpriteSheet(texture, spriteSheetPositions);
            spriteSheet.addAnimation(new AnimationStruct("INIT", AnimationStruct.makeSequence(0, 4), 6, kill));
